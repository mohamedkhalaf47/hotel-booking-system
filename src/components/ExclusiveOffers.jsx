			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12">
				{exclusiveOffers.map((exclusiveOffer) => (
					<div
						...
						style={{
							backgroundImage: `url(${exclusiveOffer.image})`,
						}}
					>
						<p className="px-3 py-1 absolute top-4 left-4 text-xs bg-white text-gray-800 font-medium rounded-full">
							{exclusiveOffer.priceOff}% OFF
						</p>
						<div>
							<p className="text-2xl font-medium font_playfair">
								{exclusiveOffer.title}
							</p>
							<p>{exclusiveOffer.description}</p>
							<p className="text-xs text-white/70 mt-3">
								Expires {exclusiveOffer.expiryDate}
							</p>
						</div>
